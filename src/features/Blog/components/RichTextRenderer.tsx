...
const options: Options = {
  renderMark: {
    [MARKS.BOLD]: (text) => <strong className="font-inter-semibold">{text}</strong>,
  ...
  renderNode: {
    [BLOCKS.PARAGRAPH]: (_node, children) => (
      <p className="font-inter text-base md:text-lg text-dark mb-6 leading-[1.7]">
        {children}
      </p>
    ),
    ...
    [BLOCKS.HEADING_2]: (_node, children) => (
      <h2 className="font-encode-bold text-dark text-2xl md:text-3xl mt-12 md:mt-16 mb-4">
        {children}
      </h2>
    ),
      ...
      if (!dims?.width || !dims?.height) {
        return (
          <div className="my-10 md:my-14 relative w-full aspect-[16/9]">
            <Image src={url} alt={alt} fill className="object-cover" sizes="(max-width: 1024px) 100vw, 768px" />
          </div>
        );
      }
